...
__global__ void sgemm_kernel_v3(float *A, float *B, float *C, int M, int N, int K, float alpha, float beta) {
    ...
    // staging regs for A global vector loads
...
int main() {
    ...
    for (int i = 0; i < warmup_iters; ++i) {
        sgemm_kernel_v3<BM, BN, BK, TM, TN><<<grid, block>>>(d_A, d_B, d_C, M, N, K, alpha, beta);
    }
