...
1. 请详细解释一下 Transformer 模型中的自注意力机制是如何工作的？
   ...
   - 计算注意力打分矩阵： $$S=\frac{QK^\top}{\sqrt{d_k}}\quad\in\mathbb{R}^{L\times L}$$
   - 对每一行做 softmax： $$A = softmax(S) \quad (行和为 1)$$
   - 输出： $$Y = AV$$ 其中， $\sqrt{d_k}$ 是为了让点积的尺度更稳定（否则 $d_k$ 大时 logits 方差变大，softmax 易饱和）。
   - 应用Mask（尤其是自回归模型）
...
2. 什么是位置编码？为什么在 Transformer 必需？
   ...
   - 自注意力本身对 token 的排列是 **置换不变（permutation equivariant）** 的。如果把输入序列整体打乱顺序，同样的 Q/K/V 计算与点积结构并不会"天然"知道谁在前谁在后（它只看向量内容）。
